...
def read_curve():
    ...
    for dirs in range(2):
        ...
        for i in range(6):
            ...
            #print(i)
            #print(curve_points_v)
            #print(curve_points_i)
            #root_dir = os.path.join(output_path, panel_name.get())
            
            folder_path = 'curves/' + panel_name.get()
            ...
            if(dirs == 0):
                ...
                dir_text = 'Forward'
            file_name = os.path.join(folder_path, 'pix_' + str(i) + '_' + dir_text + '.csv')    
      
            with open(file_name, mode='w', newline='', encoding='utf-8') as file:
                for j in range(0, len(curve_points_v[i]) - 1):
                    #print(curve_points_i[i][j])
                    file.write(str(curve_points_v[i][j]) + ',' + str(curve_points_i[i][j]) + '\n')
